		...
		public static void DrawMatchStats(ChallengeController controller)
        {
            if (controller.PlayerWhite.IsBot && controller.PlayerBlack.IsBot)
            {
                ...
                void DrawStats(ChallengeController.BotMatchStats stats)
                {
					...
					DrawNextText($"Wins: {stats.NumWins}", regularFontSize, new Color(45, 237, 96, 255));
					DrawNextText($"Draws: {stats.NumDraws}", regularFontSize, new Color(239, 242, 75, 255));
					DrawNextText($"Losses: {stats.NumLosses}", regularFontSize, new Color(224, 56, 40, 255));
